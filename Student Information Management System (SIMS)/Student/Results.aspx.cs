...
namespace Student_Information_Management_System__SIMS_.Student
{
    public partial class Results : Page
    {
        ...
        private void LoadGradeRecords()
        {
            ...
            displayDt.Columns.Add("FinalPercentage", typeof(double));
            displayDt.Columns.Add("CalculatedGrade", typeof(string));

            double filteredQualityPoints = 0;
            int filteredAttemptedCredits = 0;
            int totalEarnedCredits = 0; // Cumulative credits passed within this filter selection context
            ...
            foreach (DataRow row in rawDt.Rows)
            {
                ...
                // Earned credits only count passed courses (grade is not F)
                if (grade != "F")
                {
                    totalEarnedCredits += credits;
                }

                // Attempted credits must include all modules taken under this filter view context
        ...
        {
            // Completely ignores Session or Semester input parameters to calculate real all-time performance
            string sqlAllTime = @"
                SELECT 
                    c.Credits,
                    (SUM((CAST(g.MarksObtained AS DECIMAL(5,2)) / NULLIF(CAST(g.MaxMarks AS DECIMAL(5,2)), 0)) * g.WeightPercentage) / NULLIF(SUM(g.WeightPercentage), 0)) * 100 AS FinalPercentage
            ...
            // Bind directly to the frontend web control component metric identifier label instance
            if (lblCGPA != null)
            {
